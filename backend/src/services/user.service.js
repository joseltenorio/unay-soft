...
async function updateUser(idEstablecimiento, idUsuario, userData) {
  ...
  if (duplicatedResult.rows.length > 0) {
    ...
    if (duplicatedUser.email.toLowerCase() === normalizedEmail) {
      const error = new Error("Ya existe otro usuario registrado con ese correo.")
      error.statusCode = 409
      throw error
    }

    if (duplicatedUser.username.toLowerCase() === normalizedUsername) {
      const error = new Error("Ya existe otro usuario registrado con ese nombre de usuario.")
      error.statusCode = 409
      throw error
    }
  }
